 ...
 * Rules Engine
 *
 * @package GamiPress\Kali-Forms\Rules_Engine
 * @since 1.0.0
 */
 ...
 */
function gamipress_kali_forms_user_has_access_to_achievement( $return = false, $user_id = 0, $requirement_id = 0, $trigger = '', $site_id = 0, $args = array() ) {
    ...
    if( $return && ( $trigger === 'gamipress_kali_forms_field_value_submission'
        || $trigger === 'gamipress_kali_forms_specific_field_value_submission' ) ) {
        
        $field_name = $args[2];
        $field_value = $args[3];

        $required_field_name = gamipress_get_post_meta( $requirement_id, '_gamipress_kali_forms_field_name', true );
        $required_field_value = gamipress_get_post_meta( $requirement_id, '_gamipress_kali_forms_field_value', true );
...
add_filter( 'user_has_access_to_achievement', 'gamipress_kali_forms_user_has_access_to_achievement', 10, 6 );