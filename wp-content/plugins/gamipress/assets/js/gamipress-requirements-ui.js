var gamipress_requirements_unsaved_changes = false;
...
(function($) {
    ...
    // Check if any field has changed their values
    $('.requirements-list').on( 'change', 'input, select, textarea', function() {
        ...
        if( has_unsaved_changes && ! row.find('.requirement-header-title .requirement-unsaved-changes').length ) {
            ...
            gamipress_requirements_unsaved_changes = true;
        }
    ...
    // Check on save post (block editor)
    // Note: On block editor, the button is added through JS, so we need to wait some time to get it inside
    setTimeout(function() {
        $('button.editor-post-publish-button').on( 'click', function() {
            if( gamipress_requirements_unsaved_changes ) {
                $('.gamipress-save-requirements').trigger('click');
            }
        } );
    }, 2000 );
...
function gamipress_update_requirements( element, loop ) {
    ...
    $.post(
        ...
        function( response ) {
            ...
            gamipress_requirements_unsaved_changes = false;
